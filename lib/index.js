  ...
  return function(files, metalsmith, done){
    ...
    function convert(file, done){
      ...
      if (useExtends || clone.extends) {
        ...
        clone.filename = path.join(metalsmith.directory(), dir, (file + "-" + clone.extends).replace(/\//g, '_'));

        str = util.format(extendsPattern, clone.extends);

        if (defaultBlock) {
          testBlockPattern = util.format(blockPattern[0], defaultBlock);
          if (clone.contents.indexOf(testBlockPattern) === -1) {
            str += testBlockPattern;
            testBlockPattern = true;
          }
        }

        str += clone.contents;

        if (defaultBlock && testBlockPattern === true && blockPattern.length > 1) {
